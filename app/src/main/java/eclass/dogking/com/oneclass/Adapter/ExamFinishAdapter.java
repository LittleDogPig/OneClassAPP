...
import eclass.dogking.com.oneclass.ExamActivity;
import eclass.dogking.com.oneclass.ExamFinishActivity;
...
public class ExamFinishAdapter extends RecyclerView.Adapter<ExamFinishAdapter.ViewHolder> {
    ...
    public ExamFinishAdapter(Context context,List<LectureShow> lectureShows,String tel) {
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, int position) {
        ...
        Glide.with(mContext).load(OneclassUtils.getBaseURL()+lectureShow.getPictureUrl()).asBitmap().centerCrop().placeholder(R.mipmap.loading).centerCrop().error(R.mipmap.error).into(viewHolder.i1);
//.override(100, 100) .fitCenter()
        viewHolder.itemView.setOnClickListener(new View.OnClickListener(){
            public void  onClick(View v){
                int position =viewHolder.getAdapterPosition();
                LectureShow lectureShow1=lectureShows.get(position);
                Intent intent = new Intent(v.getContext(),ExamFinishActivity.class);
